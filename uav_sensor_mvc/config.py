...
MOTION_LABELS = {
    "normal":   "обычный",     # почти прямой / короткий путь (1–3 точки)
    "mixed":    "смешанный",   # средний разброс (3–7 точек)
    "complex":  "сложный",     # зигзаг, максимально извилисто (6–15 точек)
    "balanced": "сбаланс.(микс)",  # случайный выбор режима каждую итерацию (есть прямые)
}
...
class Params:
    ...
    n_cap: int = 15                # потолок числа путевых точек манёвра/ломаной
