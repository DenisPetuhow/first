...
# ФАЙЛ ОБЛАСТИ `область.txt` (план 10 §10.5): config читает область из него, а реестр
# THREAT_AREAS — лишь запасной путь; без файла программа молча открылась бы по реестру.
datas.append((os.path.join(_geo_src, "область.txt"), _geo_dst))

# ⚠️ ПРАВИЛО ЗАКАЗЧИКА 16.09.2026 — В СБОРКУ ИДЁТ ТОЛЬКО ОБЛАСТЬ, НАЗВАННАЯ ЗАКАЗЧИКОМ
# (СБОРКА_EXE §0б). Её слои, рельеф и тайлы — выше и ниже; ЧУЖИЕ ОБЛАСТИ (большая 3.Marshut
# и прочие) не кладутся, `geo_cache/ui_state.json` (память выбора области) — тоже: с ним
# программа открылась бы не на той области.
#
# КАРТИНКИ КАРТЫ — только сами файлы из geo_cache/карты_картинки, БЕЗ подпапки
# «детализация». Та — кэш резкости (`view_qt/map_image.py` :: `DETAIL_DIR`, `build_detail`):
# его строит кнопка в окне «Карта картинкой» уже на месте, картинка и без него показывается,
# а весит он больше самих картинок (1 530 МБ против 1 385 МБ, замер 16.09.2026).
_img_src = os.path.join(ROOT, "geo_cache", "карты_картинки")
if os.path.isdir(_img_src):
    for _name in sorted(os.listdir(_img_src)):
        _p = os.path.join(_img_src, _name)
        if os.path.isfile(_p):                     # файл картинки; папка «детализация» — мимо
            datas.append((_p, os.path.join("geo_cache", "карты_картинки")))
