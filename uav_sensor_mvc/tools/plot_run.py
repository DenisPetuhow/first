# -*- coding: utf-8 -*-
"""КАРТИНКИ ПРОГОНА — проверка глазами того, чего числа не показывают.

ТРИ ВИДА (`--fig`):
    belts  — ⭐ облёт цели: маршруты по повадкам, пояса, столбцы «заказано → вышло»;
    spread — разброс: один режим на весовой карте, область залёта, рубежи подхода;
    shape  — форма: только маршруты с дефектом (развороты точкой, возвраты).
...
⚠️ Картинка не доказывает, а показывает: глаз не отличит 26.7 % от 24.9 %. Сначала числа
(`motion_check`), картинка — когда они сошлись, а поведение всё равно странное.
⚠️ Сценарий тот же, что у эталона движения (`build_scenario`), иначе картинку не с чем
сравнить. Зерно закреплено — картинка повторяется дословно.
Кладёт в `теория/замеры_моделирования/картинки/`. Разбор — теория/код/КАРТИНКИ_ПРОГОНА.md.
"""
import argparse
import os
import sys
...
def _find_root(markers=("CLAUDE.md", "config.py")):
    ...
    # ⚠️ Не «подняться на N уровней»: это врёт молча при каждом переносе файла.
...
def collect(m, level, seed, routes):
    ...
    # число маршрутов. Отдаёт: список (линия (N,2) км, повадка, пояс или −1).
    # ⚠️ Не `run_batch`: метки лежат в `_stats` ПОСЛЕДНЕГО розыгрыша — читать сразу.
