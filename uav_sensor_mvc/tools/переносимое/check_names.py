...
EXTERNAL = {
    # ⚠️ УЧЕБНЫЕ ПРИМЕРЫ. В 8_КОММЕНТАРИИ_В_КОДЕ.md показан ОБРАЗЕЦ оформления на
    # вымышленной функции (заготовка под задачу 10.1) — её переменных в коде нет и быть
    # не должно. Пример помечен в самом документе; фильтр гасит ложную находку.
    "over",
    # ⚠️ ПРОЕКТИРУЕМЫЕ ИМЕНА — временная группа, СНИМАТЬ ПО МЕРЕ РЕАЛИЗАЦИИ. План
    # описывает механизм, которого ещё нет, и называет его будущим именем: без этого
    # постановку не написать. Признак, что запись пора убрать, — имя появилось в коде.
    # (`TileLayer` снят 13.09.2026: класс реализован в `view_qt/tile_layer.py`.)
    # ЗНАЧЕНИЯ И КЛЮЧИ ТЕГОВ OSM, названные ради того, что мы их НЕ берём или по ним НЕ
    # фильтруем (МЕТОДИЧКА_КАРТА_УГРОЗ §2.1, план 10.11): в нашем коде их нет по замыслу.
    "access", "surface", "tracktype", "bridleway", "construction", "living_street",
    "steps",
    # метод pyqtgraph, от которого отказались в пользу `set_span_x` (ПОКАЗ_ТАЙЛОВ §4а)
    "scaleBy",
    # УДАЛЁННЫЕ 13.09.2026 поле «район + 100 км» и его функция: называются в ИСТОРИИ
    # («было») — ПРЕДЕЛ_ВИДА, задача 10.8, ОГРАНИЧЕНИЯ 5.1п, архив журнала. Там им и место.
    "THREAT_VIEW_PAD_KM", "_pad_box",
    # УДАЛЁННЫЕ 16.09.2026 кольца облёта (заменены поясами в долях, план 11, 11.4):
    # называются в ИСТОРИИ решения — план 11 §11.3.3–11.3.4, МАРШРУТЫ_ИСПРАВЛЕНИЕ §5.3.
    "_orbit_ring", "THREAT_ORBIT_RADII_KM",
    # ИМЕНА ФАЙЛОВ ДАННЫХ области Marshut (куски, скачанные заказчиком 13.09.2026) и
    # ВНУТРЕННИЕ поля `pyrosm` 0.11, на которые сослались, объясняя, что файл читается один
    # раз (УПАКОВКА_ОБЛАСТИ, план 10.13). В нашем коде их нет и быть не должно.
    "Mar_1", "Mar_2", "Mar_3", "_nodes", "_way_records",
    ...
    # ИМЕНА, НАЗВАННЫЕ РАДИ ОТКАЗА ОТ НИХ. В документации сказано, чего мы НЕ делаем и
    # почему: `DeviceCoordinateCache` — режим кэша Qt, выключенный намеренно (он мылит
    # вектор при зуме), `pyarrow` — библиотека, которой в окружении нет, из-за чего не
    # берём GeoParquet. Обоих в коде нет ПО ЗАМЫСЛУ, и это не расхождение.
    "DeviceCoordinateCache", "pyarrow",
    ...
    "InputWindow", "UAV_TILE_AREA", "SensorSpec", "manual_sensors",
