...
VARIANT = re.compile(
    ...
    r"снят при|снят \d|с одной поправкой|с поправкой|"
    # ПРИМЕР КОМАНДЫ с подменой параметра: `--set THREAT_ORBIT_DIRS=24` — это показ
    # того, КАК запустить опыт с другим значением, а не утверждение о настройке.
    # Строка «=24» иначе читается как «в тексте 24, в коде 12» и даёт ложную тревогу
    r"--set|--params", re.IGNORECASE)
