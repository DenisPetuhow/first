...
from config import (THEME, THREAT_LAYERS, THREAT_LAYER_ORDER, MODE_LABELS,
                    ...
                    THREAT_SPEND_LABELS, DEMO_MAP_LAYERS, DEMO_FORCE_OFFLINE)
...
class ThreatMapView(QtWidgets.QWidget, BasemapMixin):
    ...
    # ---- окно «Справка» ----
    def _open_help(self):
        """Открыть справку. Не модальное окно: читать можно, глядя на карту."""
        if getattr(self, "_help_dlg", None) is None:
    ...
    def _build_ui(self, params):
        ...
        self.btn_help = QtWidgets.QPushButton("❓ Справка — как пользоваться")
        self.btn_help.setToolTip("Механика программы, что делает каждая кнопка, "
                                 "ограничения модели, как параметры влияют на расстановку "
                                 "датчиков и на движение БПЛА.")
        self._tint(self.btn_help, THEME["accent2"])
    ...
    def _build_map_combo(self, col, params):
        ...
        # ДЕМОНСТРАЦИОННАЯ СБОРКА: список подложек сужается до тех, что заведомо есть в
        # кэше. Остальные ничего не покажут на компьютере без интернета — предлагать их
        # значит обещать то, чего не будет.
        if DEMO_MAP_LAYERS:
            keep = [k for k in self._map_keys if k in DEMO_MAP_LAYERS]
            if keep:
                self._map_keys = keep
        ...
        # в демонстрационной сборке офлайн включён и не снимается: сети на чужом ПК может
        # не быть вовсе, а ожидание тайлов из сети выглядит как зависание
        offline = bool(getattr(params, "map_offline3", False)) or DEMO_FORCE_OFFLINE
        self.chk_offline.setChecked(offline)
        self._map_offline = offline
        params.map_offline3 = offline
        if DEMO_FORCE_OFFLINE:
            self.chk_offline.setEnabled(False)
            self.chk_offline.setToolTip("В демонстрационной сборке подложка берётся только "
                                        "из локального кэша — интернет не нужен.")
...
HELP_HTML = """
<h2>Карта угроз — руководство пользователя</h2>
<p><i>Программа определяет расположение датчиков обнаружения, обеспечивающее засечку
беспилотных аппаратов при минимальном числе датчиков. Решение рассчитывается численно
по данным реальной местности.</i></p>

<h3>1. Порядок работы</h3>
<ol>
<li><b>Построить карту</b> — цифровые слои (реки, дороги, ЛЭП, застройка) накладываются
    на сетку 500 м, для каждой ячейки рассчитывается вес. Вес отражает пригодность
    участка местности для полёта.</li>
<li><b>Добавить рельеф</b> — карта пересобирается с учётом высот: та же дорога в низине
    получает больший вес, чем на гребне. <i>Пересборка сбрасывает маршруты и датчики</i>,
    поскольку они рассчитаны по прежней карте.</li>
<li><b>Указать цель</b> — задаётся щелчком по карте. Точка вылета фиксирована.</li>
<li><b>Пуск / Шаг / Пакетно</b> — накопление маршрутов. «Пуск» отображает движение
    аппарата, «Пакетно» рассчитывает всю выборку сразу.</li>
<li><b>Расставить датчики</b> — размещение N датчиков в позициях, через которые проходит
    наибольшее число маршрутов.</li>
</ol>
<p><b>Применить</b> — пересчёт после изменения параметров. <b>Сброс</b> — удаление
датчиков, цели и запретных зон; карта сохраняется.</p>

<h3>2. Назначение кнопок</h3>
<table cellpadding="4">
<tr><td><b>Входные данные…</b></td><td>отдельное окно: запас хода, скорость, угол крена,
    длина прямого участка. Работу программы не блокирует</td></tr>
<tr><td><b>Выбрать цифровые карты…</b></td><td>источник данных (.npz / .osm.pbf) и состав
    слоёв, накладываемых на сетку. Влияет на вес ячеек, требует пересборки карты</td></tr>
<tr><td><b>Векторные слои…</b></td><td>выбор слоёв для отображения. Влияет только на
    отображение, пересчёт не выполняется</td></tr>
<tr><td><b>Запретная зона</b></td><td>построение области, в которой пролёт невозможен:
    щелчки задают вершины, колёсико замыкает контур, Esc отменяет построение. Маршруты
    прокладываются в обход</td></tr>
<tr><td><b>Убрать зону</b></td><td>удаление последней зоны; все зоны удаляются
    кнопкой «Сброс»</td></tr>
<tr><td><b>Весь участок</b></td><td>возврат к полному виду участка</td></tr>
</table>

<h3>3. Ограничения модели</h3>
<ul>
<li><b>Датчик не размещается на воде</b> (с буфером 250 м). В запретной зоне и в границах
    населённого пункта размещение допускается: они ограничивают полёт, а не наблюдение.</li>
<li><b>Датчики разносятся:</b> расстояние между ними не менее 0.75·R, целевое — 1.6·R.</li>
<li><b>Над городом полёт исключён.</b> Русло реки, проходящее через небольшой населённый
    пункт, остаётся коридором; в границах крупного города — закрывается.</li>
<li><b>Возвышенность выше порога исключает коридор:</b> вес такого участка обнуляется.</li>
<li><b>Длина маршрута не превышает запас хода</b>, маршрут не выходит за область залёта.</li>
<li><b>Ограничения по манёвру:</b> радиус разворота R<sub>min</sub> = V²/(g·tg φ); при
    скорости 180 км/ч и крене 25° он составляет 0.55 км. Поворот более 45° за шаг и
    разворот на месте исключены.</li>
<li><b>В зоне подхода (10 км от цели) маршрут только сближается с целью.</b> Заход с
    противоположного направления допускается, но выполняется обходом.</li>
</ul>

<h3>4. Влияние параметров на расстановку датчиков</h3>
<table cellpadding="4">
<tr><td><b>N</b></td><td>число размещаемых датчиков. Чем оно меньше, тем выше значимость
    каждой позиции и тем сильнее влияние шага сетки</td></tr>
<tr><td><b>R</b></td><td>радиус обнаружения. Больший радиус повышает засечку и снижает
    требования к сетке: при R = 6 км шаг практически не влияет, при R = 2 км влияние
    существенно</td></tr>
<tr><td><b>k</b></td><td>число датчиков, которые должны обнаружить аппарат для засечки.
    Увеличение k приводит к более плотному размещению</td></tr>
<tr><td><b>Шаг сетки</b></td><td>шаг перебора возможных позиций. Определяется
    автоматически как половина радиуса. При шаге, превышающем радиус, между позициями
    остаются промежутки шире зоны обзора: при R = 2 км и пяти датчиках засечка снижается
    с 96 % до 17 %</td></tr>
</table>

<h3>5. Влияние параметров на движение аппарата</h3>
<table cellpadding="4">
<tr><td><b>Запас хода</b></td><td>предельная протяжённость маршрута. Увеличение расширяет
    разброс маршрутов, но повышает число манёвров вблизи цели. При запасе, вдвое
    превышающем потребность, часть маршрутов не строится: избыточная дальность не может
    быть израсходована при ограничениях на манёвр</td></tr>
<tr><td><b>Разрыв между секторами</b></td><td>предельная протяжённость участка без
    ориентиров, который аппарат пересекает между коридорами</td></tr>
<tr><td><b>Уход от ориентира</b></td><td>допустимое боковое отклонение от реки или дороги.
    Увеличение снижает средний вес под маршрутом</td></tr>
<tr><td><b>Длина прямого участка</b></td><td>протяжённость удержания курса до
    доворота. Увеличение выпрямляет маршрут</td></tr>
<tr><td><b>Скорость и крен</b></td><td>определяют радиус разворота и, следовательно,
    манёвренность аппарата</td></tr>
</table>

<h3>6. Режимы расчёта</h3>
<table cellpadding="4">
<tr><td><b>Приоритет по весу</b></td><td><i>максимальный</i> — маршрут стремится к
    коридорам с высоким весом (реки, дороги); <i>минимальный</i> — к участкам с низким
    весом; <i>средний</i> — вероятность пропорциональна весу; <i>смесь</i> — режим
    выбирается для каждого маршрута отдельно</td></tr>
<tr><td><b>Разброс по карте</b></td><td>удаление маршрута от прямой вход—цель:
    <i>центр</i> / <i>середина</i> / <i>края</i> / <i>смесь</i>. Разброс определяет
    покрытие области выборкой</td></tr>
<tr><td><b>Расход запаса</b></td><td>порядок использования запаса хода: израсходовать в
    начале маршрута, ближе к цели либо равномерно по всей протяжённости</td></tr>
<tr><td><b>Режим оптимизации</b></td><td>критерий расстановки: покрытие, стоимость или
    их баланс</td></tr>
</table>

<h3>7. Отображаемые слои</h3>
<p><b>Весовая карта</b> — пригодность местности для полёта.
<b>Зона пролёта</b> — все участки, достижимые в пределах запаса хода.
<b>Маршруты движения</b> — накопленная выборка путей.
<b>Тепловая карта маршрутов</b> — частота пролёта над ячейкой; расстановка датчиков
выполняется по ней. <b>Приоритет высот</b> — результат учёта рельефа в весе: синим
обозначены укрытия, красным — возвышенности, чёрным — исключённые коридоры.</p>
<p>Отметка слоя управляет только отображением и не влияет на расчёт. Ползунок под группой
задаёт прозрачность. Слой, который ещё не рассчитан, отображается неактивным.</p>
"""


class HelpDialog(QtWidgets.QDialog):
    """Окно «Справка»: механика, кнопки, ограничения, влияние параметров, режимы.

    Не модальное — с ним можно работать, глядя на карту. Текст лежит в HELP_HTML рядом,
    чтобы правился без раскопок по коду."""

    def __init__(self, parent):
        super().__init__(parent)
        self.setWindowTitle("Справка — карта угроз")
        ...
        view.setStyleSheet(
            "QTextBrowser { background: #f7f9fb; color: #10202f; "
            "border: 1px solid %s; border-radius: 6px; padding: 8px; }" % THEME["grid"])
